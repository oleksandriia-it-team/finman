...
* **`entities/`**: **Shared Kernel (Домен та Доступ до даних / Аналог шару Entities у FSD)**.
    ...
    * **Репозиторії**: «Тупі» (dumb) сервіси доступу до даних, що містять атомарні CRUD-операції (`findById`, `save`, `update`).
    * Містять чисту бізнес-логіку на рівні самої сутності (багаті моделі / rich models), але ЖОДНОЇ оркестрації чи взаємодії між кількома різними сутностями.
...
* **`features/`**: **Ізольовані бізнес-сценарії (Use Cases / Прецеденти)**.
    ...
    * **Use Cases**: Оркестратори, які інжектять (впроваджують) спільні репозиторії з папки `entities/` для виконання складної логіки та управління транзакціями.
    * **DTO та Валідація**: Вхідні контракти та схеми валідації, специфічні для запиту конкретної фічі.

* **`shared/`**: **Серверні утиліти**.
    * Наскрізний функціонал (cross-cutting concerns) та інфраструктурні помічники (наприклад, логери, кастомні класи помилок, форматери дат, базові класи).
---
...
* **`domains/`**: **Спільна Бізнес-логіка**. Згрупована за бізнес-доменами. Містить код, що використовується І Клієнтом, І Сервером.
