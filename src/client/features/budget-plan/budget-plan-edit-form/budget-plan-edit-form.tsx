...
export function BudgetPlanEditForm({ initialData, onSuccess, onCancel }: Readonly<BudgetPlanEditFormProps>) {
  ...
  const otherEntriesRaw = methods.watch('otherEntries');
  const watchedOtherEntries = useMemo(
    () =>
      (otherEntriesRaw ?? []) as Array<{
        id?: number;
        title: string;
        sum?: number;
        type: string;
  ...
  const otherEntriesForSection = otherEntriesWithIndex
    ...
    .map(({ fieldId, entry }) => ({
      ...
      title: entry?.title ?? '',
      sum: entry?.sum ?? 0,
      type: (entry?.type as 'income' | 'expense' | undefined) ?? 'expense',
